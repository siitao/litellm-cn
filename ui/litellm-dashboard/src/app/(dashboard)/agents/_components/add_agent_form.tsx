...
const AddAgentForm: React.FC<AddAgentFormProps> = ({ visible, onClose, accessToken, onSuccess, teams }) => {
  ...
  useEffect(() => {
    const fetchMetadata = async () => {
      try {
        ...
        console.error(t("Error fetching agent metadata:"), error);
      }
    };
    fetchMetadata();
  }, []);
    ...
    if (currentStep === 3 && accessToken && existingKeys.length === 0) {
      const fetchKeys = async () => {
        ...
        try {
          ...
          console.error(t("Error fetching keys:"), error);
        } finally {
  ...
  useEffect(() => {
    ...
    modelAvailableCall(accessToken, userId, userRole)
      ...
      .catch((error) => {
        if (!cancelled) console.error(t("Error fetching models:"), error);
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    getAgentsList(accessToken)
      ...
      .catch((error) => {
        if (!cancelled) console.error(t("Error fetching agents:"), error);
      })
      .finally(() => {
  ...
  const handleCreateAgent = async () => {
    ...
    try {
      ...
      console.error(t("Error creating agent:"), error);
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-gray-700">{t("Require x-litellm-trace-id on calls TO this agent")}</span>
              <p className="text-xs text-gray-500 mt-1">{t("Only accept this agent being invoked with a trace-id (e.g. when used as a sub-agent).")}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-gray-700">{t("Require x-litellm-trace-id on calls BY this agent")}</span>
      ...
      <div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-gray-500">{t("Rate limits per session (x-litellm-trace-id). Each session gets its own counters.")}</p>
          <div className="grid grid-cols-2 gap-4">
          ...
          dropdownRender={(menu) => (
            <>
              ...
              <div className="px-2 py-1">
                <div className="text-xs text-gray-400 font-medium mb-1 uppercase tracking-wide px-2">{t("Not listed?")}</div>
                <div
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-amber-600">{t("For agents that don&apos;t follow a standard protocol — just needs a virtual key")}</div>
                  </div>
                </div>
              </div>
            </>
    ...
    return (
      <div>
        ...
        <div className="text-center mt-4">
          ...
          >{t("Skip for now — I&apos;ll assign a key later")}</button>
        </div>
      </div>
    );
  };
  ...
  const renderReadyStep = () => (
    <div className="text-center py-6">
      ...
      {assignedKeyAlias && (
        <p className="text-sm text-gray-600 mt-2">{t("Key")}<span className="font-medium">{assignedKeyAlias}</span>{t("has been assigned to this agent.")}</p>
      )}

    ...
    <div className="space-y-3">
      ...
      {selectedEntries.map((entry) => (
        <div key={entry.id} className="flex items-center space-x-3 p-3 border rounded-lg">
          ...
          <Button type="text" danger size="small" onClick={() => removeEntry(entry.id, entry.key)}>{t("Remove")}</Button>
        </div>
      ))}
  ...
  return (
    <div className="guardrail-optional-params">
      <div className="mb-8 pb-4 border-b border-gray-100">
        <Title level={3} className="mb-2 font-semibold text-gray-900">{t("Optional Parameters")}</Title>

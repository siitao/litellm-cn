...
function ModelInformationCell({ model, displayName }: { model: ModelData; displayName: string }) {
  ...
  const { t } = useLanguage();  const litellmModelName = model.litellm_model_name || "-";
...
function CredentialsHeader() {
  ...
  const { t } = useLanguage();  return (
    <span className="flex items-center gap-1">{t("Credentials")}<HoverCard>
        <HoverCardTrigger
          render={
            <button
              type="button"
        ...
        <HoverCardContent align="start" className="w-80">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs text-muted-foreground">{t("Credentials saved in LiteLLM that can be added to models repeatedly.")}</span>
            </div>
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs text-muted-foreground">{t("Credentials added directly during model creation or defined in the config file.")}</span>
            </div>
          </div>
        </HoverCardContent>
      </HoverCard>
...
function CostsCell({ model }: { model: ModelData }) {
  ...
  const { t } = useLanguage();  const { input_cost: inputCost, output_cost: outputCost } = model;

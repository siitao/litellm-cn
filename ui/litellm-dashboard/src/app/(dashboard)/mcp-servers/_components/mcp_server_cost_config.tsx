      ...
      <Card className="p-6">
        <div className="space-y-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <Tooltip>
              <TooltipTrigger
                render={<Info className="size-4 text-muted-foreground" aria-label={t("About cost configuration")} />}
              />
              <TooltipContent>
          ...
          <div className="space-y-4">
            <div>
              <label className="mb-2 block text-sm font-medium">{t("Default Cost per Query ($)")}<Tooltip>
                  <TooltipTrigger
                    render={
                      <Info className="ml-1 inline size-4 text-muted-foreground" aria-label={t("About the default cost")} />
                    }
                  />
              ...
              />
              <p className="mt-1 block text-sm text-muted-foreground">{t("Set a default cost for all tool calls to this server")}</p>
            </div>

            {tools.length > 0 && (
              <div className="space-y-4">
                <label className="block text-sm font-medium">{t("Tool-Specific Costs ($)")}<Tooltip>
                    <TooltipTrigger
                      render={
                        <Info className="ml-1 inline size-4 text-muted-foreground" aria-label={t("About per-tool costs")} />
                      }
                    />
                    <TooltipContent>{t("Override the default cost for specific tools. Leave blank to use the default rate.")}</TooltipContent>
                  </Tooltip>
                </label>

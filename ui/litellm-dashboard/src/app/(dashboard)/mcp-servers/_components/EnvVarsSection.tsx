  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
      <div className="flex items-center gap-2 mb-1">
        <Text strong className="text-sm">{t("Variables")}</Text>
        <Tooltip
          title={
            <>{t("Define variables you can interpolate in Static Headers or Authentication using")}{" "}
              <code>{"${VAR_NAME}"}</code>. <br />
              <b>{t("Instance")}</b>: admin-defined value used for every user.
              <br />
              <b>{t("Per-user")}</b>: each user supplies their own value (e.g. personal credentials) via the MCP Gateway
              dashboard.
            </>
          }
        >
      ...
      <Text className="text-xs text-gray-600 block mb-3">{t("Reference these in Static Headers or Authentication as")}<code>{"${VAR_NAME}"}</code>. For example:{" "}
      ...
      <Form.List name="env_vars">
        {(fields, { add, remove }) => (
          <div className="space-y-2">
            ...
            <Button type="dashed" onClick={() => add({ scope: "global" })} icon={<PlusOutlined />} block>{t("Add Variable")}</Button>
          </div>
        )}
      </Form.List>
    </div>

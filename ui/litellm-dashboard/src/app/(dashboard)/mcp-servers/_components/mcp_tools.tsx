              ...
              {hasExtraHeaders && (
                <div className="mb-4 rounded-lg border border-border bg-card p-3">
                  ...
                  {!showHeaderInput && Object.keys(passthroughHeaders).length === 0 && (
                    <p className="text-xs text-muted-foreground">{t("This server requires additional headers. Click &quot;Configure&quot; to provide values.")}</p>
                  )}
              ...
              <div className="flex flex-col flex-1 min-h-0">
                ...
                {(authorizationCodeNeedsAuth || authorizationCodeTokenRejected) && (
                  <div className="rounded-lg border border-border bg-card p-4 text-center">
                    <Lock className="mx-auto mb-2 size-6 text-muted-foreground" />
                    <p className="mb-1 text-xs font-medium">{t("Authentication required")}</p>
                    <p className="mb-3 text-xs text-muted-foreground">{t("Authenticate with the upstream provider to view available tools")}</p>
                    <Button
                      size="sm"
                  ...
                  <>
                    ...
                    {(mcpToolsResponse?.error || mcpToolsError) && !toolsAreaLoading && !toolsData.length && (
                      <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-xs text-destructive">
                        <p className="font-medium">{t("Error:")} {mcpToolsResponse?.message || (mcpToolsError as Error)?.message}
                        </p>
                      </div>
                    )}
            ...
            <div className="flex-1 overflow-auto p-4">
              {!selectedTool ? (
                ...
                <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
                  ...
                  <p className="max-w-md text-center text-sm">{t("Choose a tool from the left sidebar to start testing its functionality with custom inputs.")}</p>
                </div>
              ) : (
                /* Tool Test Panel */

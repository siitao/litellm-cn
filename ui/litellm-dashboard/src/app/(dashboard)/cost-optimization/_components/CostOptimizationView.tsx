  ...
  return (
    <div className="w-full space-y-6 p-6">
      ...
      >
        ...
        <p className="col-start-2 text-sm text-muted-foreground">{t("Have feedback? Join the discussion")}{" "}
          <a
      ...
      <Tabs defaultValue="usage" onValueChange={handleTabChange}>
        <TabsList variant="line" className="h-auto w-full justify-start rounded-none border-b p-0">
          <TabsTrigger value="usage" className="flex-none rounded-none px-4 py-2">{t("Overall")}</TabsTrigger>
          {canViewProxyWideCostData && (
            <>
              <TabsTrigger value="compression" className="flex-none rounded-none px-4 py-2">{t("Prompt Compression")}</TabsTrigger>
              <TabsTrigger value="caching" className="flex-none rounded-none px-4 py-2">{t("Prompt Caching")}</TabsTrigger>
              <TabsTrigger value="autorouter-usage" className="flex-none rounded-none px-4 py-2">{t("Auto-Router")}</TabsTrigger>
            </>
          )}
        </TabsList>

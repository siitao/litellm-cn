...
export const VectorStoreTester: React.FC<VectorStoreTesterProps> = ({ vectorStoreId, accessToken, className = "" }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      console.error(t("Error searching vector store:"), error);
      ...
      <div className="flex h-150 flex-col">
        ...
        <div className="flex items-center justify-between border-b p-4">
          ...
          {searchHistory.length > 0 && (
            <Button variant="outline" size="sm" onClick={clearHistory}>{t("Clear History")}</Button>
          )}
        </div>
                              ...
                              <div key={resultIndex} className="overflow-hidden rounded-lg border bg-muted/50">
                                ...
                                >
                                  <div className="flex items-center">
                                    ...
                                    <span className="text-sm font-medium">{t("Result")} {resultIndex + 1}</span>
                                  ...
                                  <span className="rounded-sm bg-muted px-2 py-1 text-xs text-foreground">{t("Score:")} {result.score.toFixed(4)}
                                  </span>
                                </div>
                                  ...
                                  <div className="border-t bg-card p-3">
                                    ...
                                    {result.content &&
                                      result.content.map((content, contentIndex) => (
                                        <div key={contentIndex} className="mb-3">
                                          <div className="mb-1 text-xs text-muted-foreground">{t("Content (")}{content.type})
                                          </div>
                                    ...
                                    {(result.file_id || result.filename || result.attributes) && (
                                      <div className="mt-3 border-t pt-3">
                                        <div className="mb-2 text-xs font-medium text-muted-foreground">{t("Metadata")}</div>

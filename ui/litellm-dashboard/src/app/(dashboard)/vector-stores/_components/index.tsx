...
const VectorStoreManagement: React.FC<VectorStoreProps> = ({ accessToken, userID, userRole }) => {
  ...
  const fetchCredentials = async () => {
    ...
    try {
      ...
      console.error(t("Error fetching credentials:"), error);
      ...
      <div className="gap-2 p-8 h-[75vh] w-full mt-2">
        <div className="flex justify-between mt-2 w-full items-center mb-4">
          ...
          <div className="flex items-center space-x-2">
            {lastRefreshed && <p className="text-sm text-muted-foreground">{t("Last Refreshed:")} {lastRefreshed}</p>}
        ...
        <p className="mb-4 text-sm text-muted-foreground">{t("You can use vector stores to store and retrieve LLM embeddings.")}</p>
        ...
        <Tabs defaultValue="create" onValueChange={onTabChange}>
          <TabsList variant="line" className="mb-6 h-auto w-full justify-start rounded-none border-b p-0">
            <TabsTrigger value="create" className="flex-none rounded-none px-4 py-2">{t("Create Vector Store")}</TabsTrigger>
            <TabsTrigger value="manage" className="flex-none rounded-none px-4 py-2">{t("Manage Vector Stores")}</TabsTrigger>
            <TabsTrigger value="test" className="flex-none rounded-none px-4 py-2">{t("Test Vector Store")}</TabsTrigger>
            {isProxyAdminRole(userRole || "") && (
              <TabsTrigger value="indexes" className="flex-none rounded-none px-4 py-2">{t("Indexes")}</TabsTrigger>
            )}
          </TabsList>

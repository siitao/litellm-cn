  ...
  return (
    <Card className="h-full transition-shadow hover:shadow-md">
      <CardContent className="flex h-full flex-col">
        ...
        <div className="mb-6">
          <span className="mb-2 block text-xs font-medium tracking-wider text-muted-foreground uppercase">{t("Included Guardrails")}</span>
        ...
        <Button className="mt-auto w-full" onClick={onUseTemplate}>{t("Use Template")}</Button>
      </CardContent>
    </Card>
  );
};
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        ...
        console.error(t("Error fetching policy templates:"), error);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        ...
        <Button variant="outline" onClick={onOpenAiSuggestion}>
          ...
          </svg>{t("Use AI to find templates")}</Button>
      </div>
      ...
      <div className="flex gap-6">
          ...
          <div className="w-52 shrink-0">
            <div className="sticky top-4">
              <div className="flex items-center justify-between mb-3">
                ...
                {selectedTags.size > 0 && (
                  <button onClick={handleClearAll} className="text-xs text-primary hover:underline">{t("Clear all")}</button>
                )}
              </div>
        ...
        <div className="flex-1">
          {selectedTags.size > 0 && (
            <div className="mb-4 text-sm text-muted-foreground">{t("Showing")} {filteredTemplates.length} of {templates.length} templates
            </div>
          )}
          ...
          {filteredTemplates.length === 0 && (
            <div className="py-12 text-center text-muted-foreground">
              ...
              <button onClick={handleClearAll} className="mt-2 text-sm text-primary hover:underline">{t("Clear all filters")}</button>
            </div>
          )}
        </div>
      </div>

...
import { t } from "@/i18n";const { Option } = Select2;
        ...
        <div className="mt-6">
          ...
          >
            ...
            <Card className="p-5">
              ...
              <Subtitle className="text-gray-600 mb-5">{t("Configure how requests to your domain will be forwarded to the target API")}</Subtitle>
              ...
              <div className="space-y-5">
                ...
                <Form.Item
                  label={
                    <span className="text-sm font-medium text-gray-700 flex items-center">{t("HTTP Methods (Optional)")}<Tooltip title="Select specific HTTP methods. Leave empty to support all methods (GET, POST, PUT, DELETE, PATCH). Useful when the same path needs different targets for different methods.">
                ...
                <div className="flex items-center justify-between py-3">
                  <div>
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">{t("Forward all subpaths to the target API (recommended for REST APIs)")}</div>
                  </div>
            ...
            <Card className="p-6">
              ...
              <Subtitle className="text-gray-600 mb-6">{t("Add headers that will be sent with every request to the target API")}</Subtitle>

              <Form.Item
                label={
                  <span className="text-sm font-medium text-gray-700 flex items-center">{t("Authentication Headers")}<Tooltip title={t("Authentication and other headers to forward with requests")}>
                      <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                    </Tooltip>
                  </span>
                }
            ...
            <Card className="p-6">
              ...
              <Subtitle className="text-gray-600 mb-6">{t("Add query parameters that will be automatically sent with every request to the target API")}</Subtitle>

              <Form.Item
                label={
                  <span className="text-sm font-medium text-gray-700 flex items-center">{t("Default Query Parameters (Optional)")}<Tooltip title="Query parameters that will be added to all requests. Clients can override these by providing their own values.">
                      <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                    </Tooltip>
                  </span>
                }
            ...
            <Card className="p-6">
              ...
              <Form.Item
                label={
                  <span className="text-sm font-medium text-gray-700 flex items-center">{t("Request Timeout (seconds)")}<Tooltip title="Max time to wait for the upstream API to respond. Leave empty to use general_settings.pass_through_request_timeout (default 600s).">
                ...
                name="timeout"
                extra={
                  <div className="text-xs text-gray-500 mt-2">{t("Use a higher value for slow upstream APIs (e.g. 1200 for long-running LLM calls)")}</div>
                }
              >
            ...
            <Card className="p-6">
              ...
              <Form.Item
                label={
                  <span className="text-sm font-medium text-gray-700 flex items-center">{t("Cost Per Request (USD)")}<Tooltip title={t("Optional: Track costs for requests to this endpoint")}>
                      <InfoCircleOutlined className="ml-2 text-gray-400 hover:text-gray-600" />
                    </Tooltip>
                  </span>
                }
                ...
                extra={
                  <div className="text-xs text-gray-500 mt-2">{t("The cost charged for each request through this endpoint")}</div>
                }
              >

...
export const NotificationsBell: React.FC = () => {
  ...
  const content = (
    <div className="max-w-[280px]">
      <Typography.Title level={5} className="mt-0! mb-2!">{t("LiteLLM Auto Router")}</Typography.Title>
      <Typography.Paragraph type="secondary" className="mb-3! text-sm leading-snug">{t("Route every request to the cheapest model that can handle it, no prompt changes needed.")}</Typography.Paragraph>
      <div className="flex flex-wrap items-center gap-2">
        <Button type="primary" size="small" href={AUTO_ROUTER_DOCS_URL} target="_blank" rel="noopener noreferrer">{t("Read the docs")}</Button>
        {hasUnread ? (
          <Button type="link" size="small" className="px-1!" onClick={markDismissed}>{t("Mark as read")}</Button>
        ) : null}
      </div>
    </div>
  );

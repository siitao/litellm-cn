  ...
  useEffect(() => {
    ...
    const fetchPrompts = async () => {
      try {
        ...
        console.error(t("Failed to fetch prompts:"), error);
      }
    };
  ...
  useEffect(() => {
    const fetchPossibleRoles = async () => {
      try {
        ...
        console.error(t("Error fetching possible user roles:"), error);
      }
    };
  ...
  const handleCreate = async (formValues: Record<string, any>) => {
    try {
      ...
      try {
        ...
        console.error(t("Error parsing metadata:"), error);
      }
      ...
      <Modal open={isModalVisible} width={1000} footer={null} onOk={handleOk} onCancel={handleCancel}>
        <Form form={form} onFinish={handleCreate} labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} labelAlign="left">
          ...
          <div className="mb-8">
            ...
            >
              <Radio.Group onChange={(e) => setKeyOwner(e.target.value)} value={keyOwner}>
                ...
                <Radio value="agent">{t("Agent")}<Tag color="purple">{t("New")}</Tag>
                </Radio>
              </Radio.Group>
            </Form.Item>
              ...
              >
                <div>
                  <div style={{ display: "flex", marginBottom: "8px" }}>
                    ...
                    />
                    <Button2 onClick={() => setIsCreateUserModalVisible(true)} style={{ marginLeft: "8px" }}>{t("Create User")}</Button2>
                  </div>
            ...
            {keyOwner === "agent" && (
              <div className="mt-4 p-4 bg-purple-50 border border-purple-200 rounded-md">
                <div className="mb-3">
                  <span className="text-sm font-medium text-gray-700">{t("Select Agent")}<span className="text-red-500">*</span>
                  </span>
                </div>
                <Select
                  showSearch
                ...
                />
                <div className="text-xs text-gray-500 mt-2">{t("This key will be used by the selected agent to make requests to LiteLLM")}</div>
              </div>
            )}
            <Form.Item
              label={
              ...
              >
                ...
                >
                  {!selectedProjectId && selectedCreateKeyTeam && (
                    <Option key="all-team-models" value="all-team-models">{t("All Team Models")}</Option>
                  )}
                  {!selectedProjectId && !selectedCreateKeyTeam && (
                    <Option key="all-proxy-models" value="all-proxy-models">{t("All Proxy Models")}</Option>
                  )}
                ...
                >
                  <Option value="llm_api" label={t("AI APIs")}>
                    <div style={{ padding: "4px 0" }}>
                      ...
                      <Typography.Paragraph type="secondary" style={{ fontSize: 11, margin: "2px 0 0" }}>{t("Can call only AI API routes (chat/completions, embeddings, etc.)")}</Typography.Paragraph>
                    </div>
                  </Option>
                  <Option value="management" label={t("Management")}>
                    <div style={{ padding: "4px 0" }}>
                      <Typography.Text strong>{t("Management")}</Typography.Text>
                      <Typography.Paragraph type="secondary" style={{ fontSize: 11, margin: "2px 0 0" }}>{t("Can call only management routes (user/team/key management)")}</Typography.Paragraph>
                    </div>
                  </Option>
                  <Option value="default" label={t("Full Access")}>
                    <div style={{ padding: "4px 0" }}>
                      <Typography.Text strong>{t("Full Access")}</Typography.Text>
                      <Typography.Paragraph type="secondary" style={{ fontSize: 11, margin: "2px 0 0" }}>{t("Can call all routes (AI APIs, Management, and read-only)")}</Typography.Paragraph>
                    </div>
                  </Option>
                </Select>
              </Form.Item>
              ...
              <Accordion className="mt-4 mb-4">
                ...
                <AccordionBody>
                  ...
                  <Form.Item
                    className="mt-4"
                    label={
                      <span>{t("Tokens per minute Limit (TPM)")}{" "}
                  ...
                  <Form.Item
                    className="mt-4"
                    label={
                      <span>{t("Requests per minute Limit (RPM)")}{" "}
                  ...
                  <Form.Item
                    className="mt-4"
                    label={
                      <span>{t("Throttle on budget exceeded")}{" "}
                  ...
                  <Form.Item
                    label={
                      <span>{t("Disable Global Guardrails")}{" "}
                  ...
                  {canViewPolicies && (
                    <Form.Item
                      label={
                        <span>{t("Policies")}{" "}
                          <Tooltip title={t("Apply policies to this key to control guardrails and other settings")}>
                            <a
                  ...
                  <Form.Item
                    label={
                      <span>{t("Allowed Pass Through Routes")}{" "}
                  ...
                  {premiumUser ? (
                    ...
                    <Tooltip
                      title={
                        <span>{t("Key-level logging settings is an enterprise feature, get in touch -")}<a href="https://www.litellm.ai/enterprise" target="_blank">
                  ...
                  <Accordion className="mt-4 mb-4">
                    <AccordionHeader>
                      <div className="flex items-center gap-2">
                        ...
                        <Tooltip
                          title={
                            <span>{t("Learn more about advanced settings in our")}{" "}
                              <a
                                href={
                                  proxyBaseUrl
          ...
          <div style={{ textAlign: "right", marginTop: "10px" }}>
            <Button2 htmlType="submit" disabled={isFormDisabled} style={{ opacity: isFormDisabled ? 0.5 : 1 }}>{t("Create Key")}</Button2>
          </div>
        </Form>
      </Modal>

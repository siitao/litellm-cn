...
export function RegenerateKeyModal({ selectedToken, visible, onClose, onKeyUpdate }: RegenerateKeyModalProps) {
  ...
  const handleRegenerateKey = async () => {
    ...
    try {
      ...
      console.error(t("Error regenerating key:"), error);
        ...
        regeneratedKey
          ...
          : [
              <Space key="footer-actions">
                ...
                <Button type="primary" icon={<SyncOutlined />} onClick={handleRegenerateKey} loading={isRegenerating}>{t("Regenerate")}</Button>
              </Space>,
            ]
      }
    >
      {regeneratedKey ? (
        <Flex vertical gap="middle">
          <Alert type="warning" showIcon message={t("Save it now, you will not see it again")} />

          <Flex vertical gap={2}>
            <Text type="secondary" style={{ fontSize: 12 }}>{t("Key Alias")}</Text>
            <Text>{selectedToken?.key_alias || "No alias set"}</Text>
          </Flex>

          <Flex vertical gap={6}>
            <Text type="secondary" style={{ fontSize: 12 }}>{t("Virtual Key")}</Text>
            <div
              style={{
                background: "#f5f5f5",
          ...
          <Row gutter={12}>
            <Col span={12}>
              <Form.Item
                ...
                extra={
                  <Flex vertical gap={2}>
                    <Text type={keyIsExpired ? "danger" : "secondary"} style={{ fontSize: 12 }}>{t("Current expiry:")} {selectedToken?.expires ? formatExpiresUtc(selectedToken.expires) : "Never"}
                      {keyIsExpired && " (expired)"}
                    </Text>
                    {newExpiryTime && (
                      <Text type="success" style={{ fontSize: 12 }}>{t("New expiry:")} {newExpiryTime}
                      </Text>
                    )}
                  </Flex>
                }
              >
                <Input placeholder={t("e.g. 30s, 30h, 30d")} />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item
                ...
                extra={
                  <Text type="secondary" style={{ fontSize: 12 }}>{t("Recommended: 24h to 72h for production keys")}</Text>
                }
                rules={[DURATION_RULE]}
              >
                <Input placeholder={t("e.g. 24h, 2d")} />
              </Form.Item>
            </Col>
          </Row>
        </Form>

    ...
    <div className="bg-white rounded-lg shadow-sm w-full max-w-full overflow-hidden mb-6">
      <Collapse
        ...
        items={[
          {
            key: "1",
            label: <h3 className="text-lg font-medium text-gray-900">{t("Vector Store Requests")}</h3>,
            children: (
              <div className="p-4">
                {data.map((request, index) => (
                  <div key={index} className="mb-6 last:mb-0">
                    <div className="bg-white rounded-lg border p-4 mb-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <div className="flex">
                            <span className="font-medium w-1/3">{t("Query:")}</span>
                            <span className="font-mono">{request.query}</span>
                          </div>
                          <div className="flex">
                            <span className="font-medium w-1/3">{t("Vector Store ID:")}</span>
                    ...
                    <h4 className="font-medium mb-2">{t("Search Results")}</h4>
                    <div className="space-y-2">
                          ...
                          <div key={resultIndex} className="border rounded-lg overflow-hidden">
                            ...
                            >
                              ...
                              <div className="flex items-center">
                                <span className="font-medium mr-2">{t("Result")} {resultIndex + 1}</span>
                                <span className="text-gray-500 text-sm">{t("Score:")}<span className="font-mono">{result.score.toFixed(4)}</span>
                                </span>
                              </div>
                            </div>

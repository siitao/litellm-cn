...
const MakeModelPublicForm: React.FC<MakeModelPublicFormProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(t("Error making model groups public:"), error);
  ...
  const renderStep2Content = () => {
    return (
      <div className="space-y-4">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <Text className="text-sm text-blue-800">{t("Total:")}<strong>{selectedModels.size}</strong> model{selectedModels.size !== 1 ? "s" : ""} will be made
            public
          </Text>
        </div>
      </div>
    ...
    return (
      <div className="flex justify-between mt-6">
        ...
        <div className="flex space-x-2">
          {currentStep === 0 && (
            <Button onClick={handleNext} disabled={selectedModels.size === 0}>{t("Next")}</Button>
          )}

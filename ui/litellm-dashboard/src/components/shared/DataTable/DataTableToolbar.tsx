...
export function DataTableToolbar<TData>({
  ...
  const { t } = useLanguage();
  const placeholder = searchPlaceholder ?? t("common.search_placeholder");
  ...
  return (
    <div className={cn("flex flex-wrap items-center justify-between gap-2", className)}>
      <div className="flex flex-1 flex-wrap items-center gap-2">
        ...
        {filters.map((filter) => (
          <Badge key={filter.id} variant="outline" className="gap-1 py-1" data-testid={`filter-chip-${filter.id}`}>
            ...
            <button
              type="button"
              aria-label={t("common.remove_filter").replace("{label}", labelFor(filter.id))}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {showViewOptions && <DataTableViewOptions table={table} label={t("common.columns")} />}
